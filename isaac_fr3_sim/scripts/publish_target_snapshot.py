...
def main():
    ...
    if not all(math.isfinite(value) for value in (args.x, args.y, args.z, args.yaw)) or args.rate <= 0:
        parser.error("x, y, z, yaw must be finite and rate must be positive")
    ...
    node.get_logger().info(
        f"Publishing snapshot {args.topic}: ({args.x:.6f}, {args.y:.6f}, {args.z:.6f}, yaw={args.yaw:.4f}) "
